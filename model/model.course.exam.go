...
func (c *CourseExam) One(ctx context.Context, db *sql.DB, LimitAnswer int) (*CourseExam, error) {
	one := &CourseExam{}
	query := `SELECT id,course_id,type_exam,exam_index,text,image_url FROM course_exam WHERE id = $1 LIMIT 1`
...
func (c *CourseExam) All(ctx context.Context, db *sql.DB, param AllCourseExam) ([]*CourseExam, error) {
	...
	query := `SELECT id,course_id,type_exam,exam_index,text,image_url FROM course_exam WHERE %s LIKE $1 AND course_id = $2 ORDER BY %s %s OFFSET $3 LIMIT $4`
